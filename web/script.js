$(document).ready(function () {
	...
	function setupImpoundForm(data) {
		...
		if (data.officer) {
			$('#officer').val(data.officer).prop('disabled', false);
		}

		if (data.mechanic) {
			$('#mechanic').val(data.mechanic).prop('disabled', false);
		}
	...
	$('#impound').click(function (event) {
		...
		releaseDate.setDate(releaseDate.getDate() + 0);
		...
		for(var i = 0; i < data.vehicles.length; i++) {
			...
			if(releasedate.getTime() > currentdate.getTime() || data.user.money < data.vehicles[i].fee || data.vehicles[i].hold_m || data.vehicles[i].hold_o) {
				button = `<td>
					...
					<button class="btn pay success" id="pay${i}" >Pay</button>
				</td></tr>`
			} else {
				button = `<td>
					<button class="btn info mr" id="info${i}">Info</button>
