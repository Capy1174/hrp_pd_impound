...
RegisterNetEvent('hrp_pd_impound:SetCharacter')
AddEventHandler('hrp_pd_impound:SetCharacter', function (playerData)
	_OwnPlayerData = playerData
end)

RegisterNetEvent('hrp_pd_impound:SetVehicleAndOwner')
AddEventHandler('hrp_pd_impound:SetVehicleAndOwner', function (vehicleAndOwner)
	_VehicleAndOwner = vehicleAndOwner;
end)

RegisterNetEvent('hrp_pd_impound:SetImpoundedVehicles')
AddEventHandler('hrp_pd_impound:SetImpoundedVehicles', function (impoundedVehicles)
	_ImpoundedVehicles = impoundedVehicles;
end)

RegisterNetEvent('hrp_pd_impound:VehicleUnimpounded')
AddEventHandler('hrp_pd_impound:VehicleUnimpounded', function (data, index)

	_ESX.ShowNotification("Your vehicle with the plate: " .. data.plate .. " has been unimpounded! check Garage A!")

end)

RegisterNetEvent('hrp_pd_impound:CannotUnimpound')
AddEventHandler('hrp_pd_impound:CannotUnimpound', function ()
...
RegisterNetEvent('hrp_pd_impound:impoundMenu')
AddEventHandler('hrp_pd_impound:impoundMenu', function ()
	...
	if (vehicle ~= nil) then
		...
		TriggerServerEvent('hrp_pd_impound:GetCharacter', _XPlayer.identifier)
		TriggerServerEvent('hrp_pd_impound:GetVehicleAndOwner', v.plate)
		Citizen.Wait(500);
...
function ShowRetrievalMenu ()
	...
	TriggerServerEvent('hrp_pd_impound:GetCharacter', _XPlayer.identifier)
	TriggerServerEvent('hrp_pd_impound:GetImpoundedVehicles', _XPlayer.identifier)
	Citizen.Wait(500)
...
RegisterNUICallback('impound', function(data, cb)
	...
	TriggerServerEvent('hrp_pd_impound:ImpoundVehicle', data)
...
RegisterNUICallback('unimpound', function(plate, cb)
	...
	TriggerServerEvent('hrp_pd_impound:UnimpoundVehicle', plate);
	DisableImpoundMenu();
	 cb('ok');
end)

RegisterNUICallback('unlock', function(plate, cb)
	TriggerServerEvent('hrp_pd_impound:UnlockVehicle', plate)
end)
----------------------------------------------------------------------------------------------------
-- Background tasks
----------------------------------------------------------------------------------------------------
